...
var convert = function (md, html, compact, wrapper, outResDir, cb){
	md = md.replace (/\\/g, "/");
    ...
    if (wrapper) {
      ...
      s = str + "<article class=\"markdown-body\">" + EOL +
        markedData + 
      "</article>" + EOL
    }
	...
	res.update (function (error){
		...
		var gen = function (md, cb){
			...
			var fileFlow = function (){
				...
				convert (p, html, args.compact, args.wrapper, outResDir, exit);
			};
